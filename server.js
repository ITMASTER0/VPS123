...
async function attachVm(vm) {
  ...
  for (let attempt = 0; attempt < 30 && !sshPortReady(vm.sshPort); attempt += 1) {
    if (!vmRunning(vm)) throw new Error(`VM ${vm.name} stopped before SSH became ready. Inspect its QEMU state and console log.`);
