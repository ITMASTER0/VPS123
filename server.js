...
function directKvmAccess() { try { fs.accessSync('/dev/kvm', fs.constants.R_OK | fs.constants.W_OK); return true; } catch { return false; } }
function canUseKvmWithSudo() { return fs.existsSync('/dev/kvm') && command('sudo', ['-n', 'true']).status === 0; }
function qemuAcceleration() { return directKvmAccess() || canUseKvmWithSudo() ? ['-enable-kvm'] : ['-accel', 'tcg,thread=multi']; }
function launchQemu(args) {
